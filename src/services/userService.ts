...
export async function registerAsGuardianAngel(
  uid: string
) {
  try {
...
export async function setGuardianAvailability(
  uid: string,
  available: boolean
) {
  try {
...
export async function updateGuardianRating(
  guardianUID: string,
  rating: number
) {
  ...
  const oldRating =
    data.guardianRating || 0;

  const count =
    data.guardianResponseCount || 0;
